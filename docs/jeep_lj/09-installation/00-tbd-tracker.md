...
| MC port threads | ⚠️ **Outlet resolved (2026-05-30):** confirmed **1/2-20 IF** per Wilwood datasheet (corrects the old 11/16-20, which was the flexline-adapter thread). Bore now fixed at **260-15542**; **confirm the inlet/reservoir thread** on that part's datasheet before ordering feed fittings (Wilwood lists inlet size but no clean thread spec). | [iBooster][ibooster] | Confirm inlet thread (260-15542) | [#21][i21] |
